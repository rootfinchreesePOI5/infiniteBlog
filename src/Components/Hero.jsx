  ...
  return latest && (
    <div className='flex flex-col md:flex-row gap-6 w-full'>
      ...
      {latest.slice(3, 4).map((item, index) => (
        <div key={index} onClick={() => {navigate(`/News/${item.title}`)}} className='cursor-pointer flex flex-col md:flex-row items-center gap-8 w-full p-4 shadow-md rounded-lg hover:shadow-lg transition-shadow duration-300'>
          {
